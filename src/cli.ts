...
function usage(): string {
  return [
    "Usage: aws-guardduty-triage <export.json> [options]",
    "",
    "Options:",
    "  --format <json|markdown|summary>  Output format (default: summary)",
    "  --now <iso>                       Override analysis time",
    "  --stale-finding-after-hours <n>  Stale finding threshold in hours (default: 48)",
...
function parseArgs(argv: string[]) {
  ...
  let staleFindingAfterHours: number | undefined;
  ...
  while (args.length > 0) {
    ...
    switch (flag) {
      ...
      case "--now":
        now = args.shift();
        break;
      case "--stale-finding-after-hours":
        staleFindingAfterHours = Number(args.shift());
        break;
      case "--fail-on-high":
        failOnHigh = true;
        break;
  ...
  return { file, format, now, staleFindingAfterHours, failOnHigh, out };
}
...
async function main() {
  const { file, format, now, staleFindingAfterHours, failOnHigh, out } = parseArgs(process.argv.slice(2));
  const input = JSON.parse(fs.readFileSync(file, "utf8")) as GuardDutyExport;
  const report = analyze(input, { now, staleFindingAfterHours });
